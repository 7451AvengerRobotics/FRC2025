package frc.robot.util.sim.motors;

import com.ctre.phoenix6.hardware.TalonFX;
import com.ctre.phoenix6.sim.TalonFXSimState;
import edu.wpi.first.math.util.Units;
import edu.wpi.first.wpilibj.RobotController;
import frc.robot.util.sim.feedback.SimFeedbackSensor;
...
public class TalonFXSim implements SimMotorController {
    ...
    public void update(final double dt) {
        ...
        for (final TalonFXSimState simState : simStates) {
            simState.setRawRotorPosition(mechanismAngularPositionRots);
            simState.setRotorVelocity(mechanismAngularVelocityRotsPerSec);
            simState.setSupplyVoltage(
                    RobotController.getBatteryVoltage() -
                            (simState.getSupplyCurrent() * 0.002)
            );
        }
    ...
    public void rawUpdate(final double mechanismPositionRots, final double mechanismVelocityRotsPerSec) {
        for (final TalonFXSimState simState : simStates) {
            simState.setRawRotorPosition(mechanismPositionRots);
            simState.setRotorVelocity(mechanismVelocityRotsPerSec);
        }
